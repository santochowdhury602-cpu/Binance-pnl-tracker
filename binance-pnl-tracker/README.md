...
- **Futures:** Binance's `/fapi/v1/income` endpoint already reports realized
  ...
  sums and buckets them. It's global by nature: no per-symbol list needed.
...
## Global coin coverage (spot)

Binance's spot trade-history endpoint requires a symbol per request — there's
no "give me everything" call — so this app supports two ways to cover it:

- **Manual list:** set `SPOT_SYMBOLS` (e.g. `BTCUSDT,ETHUSDT,SOLUSDT`) if you
  know exactly what you trade. Fast, but you have to keep it updated.
- **Auto-discovery (recommended for full coverage):** leave `SPOT_SYMBOLS`
  blank and tap **"Discover all coins"** in the app. It scans every symbol
  Binance lists (filtered to common quote assets — USDT, BUSD, USDC, FDUSD,
  BTC, ETH, BNB, TRY, EUR by default, configurable via
  `DISCOVER_QUOTE_ASSETS`), checks each for trade history, and remembers
  which ones you've actually traded. This runs in the background,
  rate-limited to ~1 request/second to stay under Binance's limits, so it
  can take several minutes the first time. After that, the regular
  **"Sync latest trades"** button only re-checks symbols already found —
  fast, no rescanning the whole exchange each time.
